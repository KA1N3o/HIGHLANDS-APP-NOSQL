...
try {
    ...
    if ($loginResponse.success) {
        ...
        Write-Host "`nStep 2: Updating role to admin..." -ForegroundColor Yellow
        ...
        Write-Host "`nStep 3: Verifying the change..." -ForegroundColor Yellow
        try {
            ...
            if ($verifyResponse.success) {
                ...
                if ($newRole -eq "admin") {
                    Write-Host "`n✅ SUCCESS! Admin role has been set correctly." -ForegroundColor Green
                } else {
                    Write-Host "`n⚠️  WARNING: Role is still '$newRole', not 'admin'." -ForegroundColor Yellow
                }
            }
        } catch {
